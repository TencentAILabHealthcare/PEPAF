...
class Saver(object):
    ...
    def save_pth(self, model, filename='checkpoint.pth'):
        self.mkdir()
        torch.save(model.state_dict(), str(self.save_dir/filename))
...
class Weighted_MSELoss(nn.Module):
    ...
    def forward(self, inputs, targets, weights=1):
        weighted_square_errors = ((inputs - targets) ** 2) * weights
        
        loss = weighted_square_errors.sum() / weights.sum()
