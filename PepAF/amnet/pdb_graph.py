"""
Adapted from
https://github.com/luoyunan/KDBNet/blob/main/kdbnet/pdb_graph.py
"""
import math
import numpy as np
import scipy as sp
...
from amnet.constants import LETTER_TO_NUM
...
def featurize_protein_graph(
    ...
    with torch.no_grad():
        ...
        epitope = torch.as_tensor(protein['epitope'], dtype=torch.float)
    ...
    data = torch_geometric.data.Data(x=X_ca, seq=seq, name=name,
                                        ...
                                        seq_emb=seq_emb, importance=epitope)
    return data


def featurize_hla_graph(
        protein, name=None,
        num_pos_emb=16, num_rbf=16,        
        contact_cutoff=8.,
    ):
    ...
    with torch.no_grad():
        coords = torch.as_tensor(protein['coords'], dtype=torch.float32)
        seq = torch.as_tensor([LETTER_TO_NUM[a] for a in protein['seq']], dtype=torch.long)        
        seq_emb = torch.load(protein['embed'])[26:200] if 'embed' in protein else None
        epitope = torch.as_tensor(protein['epitope'], dtype=torch.float)
        mask = torch.isfinite(coords.sum(dim=(1,2)))
        coords[~mask] = np.inf
